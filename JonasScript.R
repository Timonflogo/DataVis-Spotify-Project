...
for (i in 1:length(unique(df$key))) { #
  ...
  if(nrow(spotifyr::search_spotify(q = q,type = "track",limit = 1)) == 0) next
  
  #Getting ID via searching in the spotify data
  res_temp <- spotifyr::search_spotify(q = q #Name of the song
  ...
  res <- if(exists("res")) { #Check if the results object exists. if the expressions returns TRUE, then append the res object
    ...
    res <- spotifyr::search_spotify(q = q,type = "track",limit = 1)
    res <- rbind(res_temp[,c("name","id")])
  }
  
  status_key[i] <- keys[i]
  status[i] <- ifelse(nrow(spotifyr::search_spotify(q = q,type = "track",limit = 1)) != 0,yes = 1,no = 0)
  
}

success <- cbind(status_key,status)

#res <- cbind(keys,res)

#Save the file as an RDS object, you can load it into the environment using readRDS()
saveRDS(object = res
        ,file = "Jonas/SongAndIDV2.rds"#insert direction here, remember to add '.rds', example: "Jonas/SongAndID.rds"
        )
